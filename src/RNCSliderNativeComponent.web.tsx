...
const RCTSliderWebComponent = React.forwardRef(
  (
    ...
    const updateContainerPositionX = () => {
      // Use a more reliable way to get element position
      const element = containerRef.current;
      if (element && typeof element.getBoundingClientRect === 'function') {
        const rect = element.getBoundingClientRect();
        containerPositionX.current = rect.left;
    ...
    const getValueFromNativeEvent = (pageX: number) => {
      // Force position update on each drag for more accuracy
      updateContainerPositionX();
      
      const width = containerSize.current.width;
      if (!width) return value; // Safeguard against zero width
      
      // Simplify the calculation for better reliability
      const containerX = containerPositionX.current;
      const relativeX = Math.max(0, Math.min(width, pageX - containerX));
      const percentage = relativeX / width;
      
      let newValue = inverted
        ? maximumValue - percentage * (maximumValue - minimumValue)
      ...
      // Apply limits
      if (newValue < Math.max(minimumValue, lowerLimit)) {
        newValue = Math.max(minimumValue, lowerLimit);
      } else if (newValue > Math.min(maximumValue, upperLimit)) {
        newValue = Math.min(maximumValue, upperLimit);
      }
    ...
    const onTouchEnd = (event: GestureResponderEvent) => {
      ...
      const newValue = updateValue(getValueFromNativeEvent(pageX));
      animationValues.val.setValue(newValue);
    ...
    const onMove = (event: GestureResponderEvent) => {
      const pageX = event.nativeEvent.pageX;
      const newValue = getValueFromNativeEvent(pageX);
      animationValues.val.setValue(newValue);
    ...
    return (
      <View
        ...
        onResponderGrant={(event) => {
          updateContainerPositionX(); // Update position on start
